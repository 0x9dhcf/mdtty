...
1. The tag name **must** match `PROJECT_VERSION` in `CMakeLists.txt`
   (i.e. tag `v0.2.0` requires `project(mdtty VERSION 0.2.0 ...)`).
...
1. **Bump the version** in `CMakeLists.txt`:
   ```cmake
   project(mdtty VERSION X.Y.Z LANGUAGES CXX)
   ```
...
2. **Update `CHANGELOG.md`**:
   ...
   - Update the link references at the bottom of the file:
     ```markdown
     [Unreleased]: https://github.com/0x9dhcf/mdtty/compare/vX.Y.Z...HEAD
     [X.Y.Z]: https://github.com/0x9dhcf/mdtty/releases/tag/vX.Y.Z
     ```
...
6. **Watch the workflow**:
   ...
   `https://github.com/0x9dhcf/mdtty/releases/tag/vX.Y.Z` with
   `mdtty-X.Y.Z.tar.gz` attached.
...
5. Creates `mdtty-X.Y.Z.tar.gz` via `git archive` (reproducible, excludes `.git`).
